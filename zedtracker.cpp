...
//	waitKey(5);
    ...
    if(argv[2]!="NONE") writer.open(argv[1], ex1, 25, cv::Size(width, height), true);
	if(argv[3]!="NONE") video.open(argv[2], ex1, 25, cv::Size(width, height), true);
    ...
    //////////// UART initialize ///////////////////////////
    unsigned char x_byte, y_byte, z_byte, x_step, y_step, z_step , x_dir, y_dir, z_dir, xx_byte, yy_byte, xx_step, yy_step, xx_dir, yy_dir;
...
//				dilate(thresh, dilated, element);
				centre = detect.contours(smooth, dst);
				...
				if(centre.x==-1 && centre.y==-1){ //no object detected
					...
					xx_dis = 7.0 * (centre.x - 320) / width;      
					yy_dis = 7.0 * (centre.y - 240) / height;
					xx_step = (unsigned char)abs(x_dis);
					yy_step = (unsigned char)abs(y_dis);
				...
				x_dis = 7.0 * (tar1_cen.x - tar2_cen.x) / width;
				z_dis = 7.0 * (tar1_cen.y - tar2_cen.y) / height;
				x_step = (unsigned char)abs(x_dis);
				z_step = (unsigned char)abs(z_dis);
				if(x_dis > 0) x_dir = 0; //0000 0000
				else x_dir = 8; //0000 1000
				if(z_dis > 0) z_dir = 0;
				else z_dir = 8;
				...
				cout<< (int)x_byte << ", "<< (int)y_byte << ", " << (int)z_byte << ", " << (int)xx_byte << ", "<< (int)yy_byte <<endl;
